<template>
  <div class="waiting">
    ...
    >
      ...
      <div class="text-area">
        <span>
          Anda dapat langsung mengambil screenshot dan paste atau drag and drop di sini, hanya gif, jpeg, jpg, png, dan batas gambar dalam 5MB
        </span>
      </div>
    ...
    <div class="advanced" @click.stop>
      <el-tooltip
          effect="light"
          content="Jika ukuran gambar melebihi batas, maka akan otomatis dikompresi"
          placement="top-start"
      >
        <el-checkbox class="compress" v-model="compress" size="small"  @click.stop>Kompres gambar</el-checkbox>
      </el-tooltip>
    </div>
  </div>
</template>
...
const onBeforeUpload = async (raw: any) => {
  ...
  if(raw.size > MAX_SIZE){
    if(!compress.value){
      ElMessage.error('Ukuran gambar tidak boleh melebihi 5MB！')
      return false
    }else{
...
watch(paste, () => {
  ...
  if(accept.includes(type)){
    ...
    ElMessage.error('Format file salah!')
  }
})
